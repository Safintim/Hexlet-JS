...
const run = (player1, player2, cards, customRandom) => {
  const iter = (health1, name1, health2, name2, order, log) => {
    ...
    cardName = isSimpleCard(card) ? getSimpleCardName(card) : getPercentCardName(card)
    damage = isSimpleCard(card) ? simpleCardDamage(card) : percentCardDamage(card, health2)
    // END
